...
;;; by disjointness are not contradicted.

;;; Assumes the types are nonempty.

(defun check-disjointness (type1 type2 &optional (should-be-valid t))
  (is-subtypep type1 type2 nil should-be-valid)
  (is-subtypep type2 type1 nil should-be-valid)
  (is-subtypep type1 `(not ,type2) t nil)
  (is-subtypep type2 `(not ,type1) t nil)
  (is-subtypep `(and ,type1 ,type2) nil t nil)
  (is-subtypep `(and ,type2 ,type1) nil t nil)
  (is-subtypep `(and ,type1 (not ,type2)) type1 t nil)
  (is-subtypep `(and (not ,type2) ,type1) type1 t nil)
  (is-subtypep `(and ,type2 (not ,type1)) type2 t nil)
  (is-subtypep `(and (not ,type1) ,type2) type2 t nil)
  (is-subtypep type1 `(or ,type1 (not ,type2)) t nil)
  (is-subtypep type1 `(or (not ,type2) ,type1) t nil)
  (is-subtypep type2 `(or ,type2 (not ,type1)) t nil)
  (is-subtypep type2 `(or (not ,type1) ,type2) t nil)
  (is-subtypep t `(or (not ,type1) (not ,type2)) t nil)
  (is-subtypep t `(or (not ,type2) (not ,type1)) t nil)
  (is-disjointp type1 type2 t should-be-valid)
  )

(defun check-equivalence (type1 type2 &optional (should-be-valid t))
  (is-subtypep type1 type2 t should-be-valid)
  (is-subtypep type2 type1 t should-be-valid)
  (is-subtypep `(not ,type1) `(not ,type2) t)
  (is-subtypep `(not ,type2) `(not ,type1) t)
  (is-subtypep `(and ,type1 (not ,type2)) nil t)
  (is-subtypep `(and ,type2 (not ,type1)) nil t)
  (is-subtypep `(and (not ,type2) ,type1) nil t)
  (is-subtypep `(and (not ,type1) ,type2) nil t)
  (is-subtypep t `(or ,type1 (not ,type2)) t nil)
  (is-subtypep t `(or ,type2 (not ,type1)) t nil)
  (is-subtypep t `(or (not ,type2) ,type1) t nil)
  (is-subtypep t `(or (not ,type1) ,type2) t nil)
  (is-type= type1 type2 t should-be-valid))

(defun check-all-subtypep (type1 type2 &optional (should-be-valid t))
  (is-subtypep type1 type2 t should-be-valid)
  (is-subtypep `(not ,type2) `(not ,type1) t should-be-valid)
  (is-subtypep `(and ,type1 (not ,type2)) nil t should-be-valid)
  (is-subtypep t `(or (not ,type1) ,type2) t should-be-valid))

(defun check-all-not-subtypep (type1 type2 &optional (should-be-valid t))
  (is-subtypep type1 type2 nil should-be-valid)
  (is-subtypep `(not ,type2) `(not ,type1) nil))
